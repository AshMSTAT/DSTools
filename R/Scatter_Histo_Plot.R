#' @title Scatter Plot with histograms Generator
#'
...
scatter_histo_plot <- function(dataset, group, value="", x, y, ylimit){
  ...
  g <- ggplot(dataset, aes_string(x = x, y = y, color = group)) +
           geom_jitter() +
           geom_smooth(method = "loess", span = 1, se = FALSE)+
           ggtitle(c(plot_title))

  return(ggMarginal(g,type = "histogram", fill="transparent"))
